...
print_heading "Installing pre-commit hooks"
print_info "VIRTUAL_ENV=${VENV_DIR} uv run --no-sync pre-commit install"
VIRTUAL_ENV=${VENV_DIR} uv run --no-sync pre-commit install
